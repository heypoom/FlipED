...
const courseCardDispatch = (dispatch, props) => ({
  ...
  enter: () => {
    ...
    setTimeout(() => dispatch(setUi("currentView", "Course")), 150)
  }
})

const CourseCard = connect(null, courseCardDispatch)(withStyles(s)(props => (
  <Grid xs={props.xs || 12} sm={props.sm || 6} md={props.md} lg={props.lg}>
    <div>
      <Tooltip place="top" type="dark" effect="float" />
      <div className={c(s.card, s.class)}>
        <div className={s.cardBody}>
          <h2 onClick={props.enter}>{props.name}</h2>
  ...
  <div>
    ...
    <Grid r>
      <Role only="student">
        <Grid xs={12} md={4}>
          <div>
            <CourseCardHeader text="คอร์สที่พบปัญหา" danger />
            <Grid r>
